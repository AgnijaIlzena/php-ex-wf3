...
$query = $db->query('SELECT posts.id, posts.title, posts.content, posts.cover, posts.created_at, posts.category_id AS categoryId, categories.name AS category, users.lastName AS userLName, users.firstName AS userFname, DATE_FORMAT(posts.created_at, "%d %M %Y") AS date_c
FROM posts 
INNER JOIN categories ON categories.id = posts.category_id 
INNER JOIN users ON users.id = posts.user_id ORDER BY posts.created_at DESC;');
